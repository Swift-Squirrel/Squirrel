...
struct ViewErrors: ErrorHandlerProtocol {
    func getResponse(for error: Error) -> Response? {
        ...
        case .notExists(let name):
            response = try? Response(html: htmlTemplate(title: "Nut file does not exists", body: "File name: <i>\(name)</i>"))
        }
        guard let resp = response else {
            return Response(status: .internalError, body: "Error in ViewErrors: ErrorHandlerProtocol, getResponse(for:)".data(using: .utf8)!)
        }
        return resp
    }
}
...
struct ViewNutErrors: ErrorHandlerProtocol {
    func getResponse(for error: Error) -> Response? {
        ...
        case .unexpectedEnd(let reading):
            html = temp(title: "Unexpected end", body: "Unexpected end of file or using another command while reading: '\(reading)'".escaped)
        case .unknownInternalError(let commandName):
            html = temp(title: "Unknown internal error", body: "Uknown error in '\(commandName)'".escaped)
        case .wrongChainedVariable(let name, let command, let regex):
            html = temp(title: "Wrong variable name", body: "Variable '\(name)' in '\(command)' does not match regular expression: \(regex)".escaped)
        case .wrongSimpleVariable(let name, let command, let regex):
            html = temp(title: "Wrong variable name", body: "Variable '\(name)' in '\(command)' does not match regular expression: \(regex)".escaped)
        case .wrongValue(let name, let expected, let got):
            html = temp(title: "Wrong value", body: "Wrong value for '\(name)', expected type was '\(expected)' but got '\(got)'".escaped)
        }
        guard let response = try? Response(html: html) else {
            return Response(status: .internalError, body: "Error in ViewNutErrors: ErrorHandlerProtocol, getResponse(for:)".data(using: .utf8)!)
        }
        return response
    }
}
